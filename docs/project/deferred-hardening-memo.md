...
### 2026-09-07 — License selection and docs reorganization for public consumption

- Resolved the license-selection blocker recorded earlier this date and on 2026-08-31: the user explicitly directed adding a `LICENSE` and package metadata this session, so the standing "no user direction" condition that previously blocked this choice no longer applies. Added `LICENSE` (MIT), and `license`/`repository` fields to the root, adapter, and demo `package.json` files and to `Cargo.toml`/`services/databento-gateway/Cargo.toml`. Package-scope ownership (`@lwc-databento`) was already established; npm/crates.io release identity (publish credentials) remains open and out of scope for this change.
- Reorganized `docs/` for public consumption: moved the spec-driven-development process artifacts (`EXECPLAN.md`, `requirements.md`, `implementation-plan.md`, `traceability.md`, `review-route.md`, `reviewer-convergence.md`, `mise-plan.md`, `deferred-hardening-memo.md`, `dedicated-connectivity-plan.md`, `dedicated-connectivity-rfq.md`) into `docs/project/`, added `docs/project/AGENTS.md` describing that archive, and kept only developer/agent-useful reference docs (`technical-design.md`, `decision-log.md`, `test-strategy.md`, the two Lightweight Charts knowledge docs, `adr/`) directly under `docs/`. Added `docs/README.md` as the index for the retained docs.
- Updated `scripts/check-doc-contracts.sh` and every cross-reference (`AGENTS.md`, `docs/AGENTS.md`, `ARCHITECTURE.md`, `docs/technical-design.md`, `docs/decision-log.md`) to the new `docs/project/` paths; the DEC-014/015/016 traceability check for `README.md` was dropped from the gate since the rewritten public README no longer carries internal decision codes, while the check against the design/process docs is unchanged.
- Rewrote the root `README.md` for a public audience (what the project does, components, quick start, testing, documentation, contributing) and added standard OSS scaffolding: `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md` (Contributor Covenant), `SECURITY.md`, `CHANGELOG.md`, and `.github/ISSUE_TEMPLATE/`/`PULL_REQUEST_TEMPLATE.md`.
- Verified with `scripts/check-doc-contracts.sh`, `scripts/check-structure.sh`, `scripts/scan-secrets.sh`, a full-repo markdown-link check (0 broken links across 35 files), and `mise run check` (all green).
- Deferred, unchanged: broad security overhauls, penetration/soak testing, production SLO work, the wire-protocol `revision` extension, and npm/crates.io release identity remain out of scope for this change.
