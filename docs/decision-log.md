...
- Status: Proposed, revised on 2026-09-04 after a causal-map review; recommended shape is now additive, not breaking.
- Decision: Do not restructure the public API now. When a second consumer or provider makes the interface testable against two realities, add a provider-neutral `BarFeed`/`BarSink` wrapper module and a structural `bindSeries({setData, update}, …)` helper *around* the existing `DatabentoDataProvider`, non-breaking. Independently of that trigger, extend the wire protocol with a `revision` flag on `bar` events mapping to Lightweight Charts `update(bar, historicalUpdate: true)` — that is a real capability gap, not an abstraction bet. An optional TradingView Datafeed-API shim remains a possible additive outer layer.
...
- Consequence: The breaking restructuring is rejected for now; the additive wrapper is deferred behind a concrete trigger (external adopter requesting neutrality, or a planned second provider). Supersedes most of `DEC-019`'s scope — runtime schemas shrink to the wire envelope (commands, events, errors), with validated numbers branded once into `UTCTimestamp` at ingress; large-scale Rust→Zod codegen becomes mostly unnecessary. The `revision` protocol extension may proceed independently as a normal feature slice.
- Traces: `packages/databento-lightweight-charts/src/types/index.ts`, `docs/requirements.md`, `DEC-014`, `DEC-015`, `contracts/protocol-v1.md`.
