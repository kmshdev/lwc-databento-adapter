...
### DEC-019 — Generate the TypeScript wire schema from the Rust protocol types

- Status: Implemented on 2026-09-04 in reduced scope after `DEC-020`'s causal-map review.
- Decision: `services/databento-gateway/src/protocol.rs` is the machine-checked source of truth for the wire contract. `schemars` (behind the `json-schema` feature) generates `contracts/protocol-v1.schema.json`; the `protocol_contract_schema` gateway test fails when the committed schema is stale, and a TypeScript contract test (Ajv, draft 2020-12) proves every valid shared fixture satisfies the generated schema. The Zod schemas remain hand-written — no TypeScript codegen — because Zod carries semantic refinements (safe-integer times, volume/bar time equality) beyond structural schema, and codegen would couple the two build graphs against `ARCHITECTURE.md`.
- Rationale: `protocol.rs` and `schemas.ts` duplicated the same wire shapes by hand, synced only by convention and the `contracts/fixtures` behavioral tests — a real drift risk (the 2026-09-03 OSS-readiness review found one drift instance in the demo). The reduced scope avoids the harmful branches the causal-map review identified: no Rust artifacts feed the TypeScript build (the schema is a committed file plus staleness test), external contributors still develop the TypeScript package without a Rust toolchain, and the "stale generated artifact" drift vector is closed by the freshness test running inside the standard `--all-features` gate.
- Consequence: The wire contract now has a machine-readable form under `contracts/protocol-v1.schema.json`, kept fresh by `cargo test --workspace --all-features protocol_contract` and consumed by `pnpm --filter @lwc-databento/adapter test:contract`. User-directed follow-through recorded in the deferred memo: extending schema coverage with numeric range annotations (safe-integer bounds) if structural rejection of unsafe times ever needs to move into the schema itself.
- Traces: `contracts/protocol-v1.schema.json`, `services/databento-gateway/tests/protocol_contract_schema.rs`, `packages/databento-lightweight-charts/test/contract/schema-conformance.test.ts`, `contracts/AGENTS.md`.

### DEC-020 — Provider-neutral bar-feed interface typed in Lightweight Charts vocabulary

- Status: Wrapper implemented on 2026-09-04 at explicit user direction (the stated trigger for the deferred additive layer); the wire-protocol `revision` extension remains separate and not yet done.
- Decision: `packages/databento-lightweight-charts/src/feed/index.ts` adds `BarFeed`/`BarSink`/`toBarFeed`/`bindSeries`, wrapping the existing `DatabentoDataProvider` non-breakingly — the provider's own export and vocabulary (`dataset`, `stypeIn`) are unchanged, and `BarFeed` never imports or retains `ISeriesApi` (consumers pass a structural `ISeriesApiLike`). `BarEvent.historicalUpdate` exists in the TypeScript type so `bindSeries` can call `series.update(bar, historicalUpdate)`, but `toBarFeed` always leaves it `undefined` today because the wire protocol's `BarEvent` (`services/databento-gateway/src/protocol.rs`) carries no revision signal — that wire extension is unchanged from the original proposal and remains a separate follow-up.
- Rationale: Source review of `lightweight-charts` 5.2.1 confirmed core is push-only with no datafeed abstraction, so the publisher contract is "arguments to `setData`/`update`" and the adapter already emits correct Lightweight Charts types. A backward/forward causal map then tested the restructuring impulse and found it unsupported: `docs/project/requirements.md` defines "reusable" as a reusable Databento package working with Lightweight Charts data objects — already satisfied — not a provider-neutral abstraction; there are zero external consumers (repo public since 2026-09-03), so "neutrality drives adoption" is judgment, not evidence; and an opaque `SymbolRef` would hide exactly what Databento users must control (continuous-contract session pinning per `DEC-014`, parent→child selection per `DEC-015`), forcing escape hatches that reintroduce the leak. A neutral interface designed against one provider typically misfits the second anyway.
- Consequence: `DatabentoDataProvider` remains the primary export; `BarFeed`/`bindSeries` is an additive companion, so existing consumers are unaffected. The wire `revision` flag and an optional TradingView Datafeed-API shim remain open follow-ups, not required for this wrapper to be useful. Supersedes most of `DEC-019`'s original scope — runtime schemas shrink to the wire envelope (commands, events, errors), with validated numbers branded once into `UTCTimestamp` at ingress; large-scale Rust→Zod codegen was not needed (see `DEC-019`, implemented in reduced scope).
- Traces: `packages/databento-lightweight-charts/src/feed/index.ts`, `packages/databento-lightweight-charts/test/unit/feed.test.ts`, `docs/project/requirements.md`, `DEC-014`, `DEC-015`, `contracts/protocol-v1.md`.
...
- Traces: `REQ-Q-006`, `TASK-12`, `docs/project/dedicated-connectivity-plan.md`.
